...
class AnormBlogDao extends BlogDao {
  ...
  val GET_LATEST_BLOG : SqlQuery = SQL("select * from PUBLIC_BUBBLE.BLOG where publish_date <= current_date order by publish_date desc LIMIT 1")
  ...
  def getLatest : Option[Blog] = DB.withConnection{
    implicit connection =>
      ...
      Logger.debug(s"latestBlog=$optionRow")

      if (optionRow.isEmpty) {
        None
      } else {
        Some(createBlogFrom(optionRow.get))
      }
  }

  private def createBlogFrom(row : Row) : Blog = {
    Blog(
      row[Option[Long]] ("id"),
      row[String] ("title"),
      row[String] ("author"),
    ...
    DB.withConnection {
      implicit connection =>
        UPDATE_BLOG.on(
          ...
          "intro" -> blog.intro,
          "content" -> blog.content,
          "publish_date" -> blog.publishDate.asInstanceOf[java.util.Date],
          "id" -> blog.id
        ).executeUpdate()
    }
  }
